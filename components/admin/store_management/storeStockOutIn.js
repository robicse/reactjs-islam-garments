...
const storeStockOutIn = ({ endpoint, modal, handleRefress }) => {
  ...
  const [selectedFromStore, setSelectedFromStore] = React.useState(null);
  const [selectedToStore, setSelectedToStore] = React.useState(null);
 ...
 // subtotal calculation
  ...
  const handleFinalStockInCreate = async () => {
    ...
    if (!selectedFromStore) {
      return cogoToast.warn("Please Select From Store", {
        position: "top-right",
        bar: { size: "10px" },
      });
    }
    ...
    if (!selectedToStore) {
      return cogoToast.warn("Please Select To Store", {
        position: "top-right",
        bar: { size: "10px" },
      });
    }
    ...
    const body = {
      ...
      transfer_from_store_id: selectedFromStore,
      transfer_to_store_id: selectedToStore,
      ...
      <GridContainer style={{ padding: "20px 30px", marginTop: 250 }}>
        ...
        <GridItem xs={12} sm={3} md={3}>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label="From Store" variant="outlined" />
            )}
            onChange={(e, v) => setSelectedFromStore(v.id)}
          />
        </GridItem>
        ...
        <GridItem xs={12} sm={3} md={3}>
            ...
            <Autocomplete
              ...
              getOptionLabel={(option) => option.store_name}
              renderInput={(params) => (
                <TextField {...params} label="To Store" variant="outlined" />
              )}
              onChange={(e, v) => setSelectedToStore(v.id)}
            />
        ...
        <GridItem xs={12} sm={12} md={12}>
          <div style={{ marginTop: "15px" }}>
            <ProductSelectByDropdown
              ...
              storeIdRequired={true}
              idRequired={true}
              searchBody={{ store_id: selectedFromStore }}
            />
          </div>
        </GridItem>
...
export default storeStockOutIn;
