...
const StockOutComponent = ({ endpoint, modal, handleRefress }) => {
  ...
  useAsyncEffect(async (isMounted) => {
    try {
      const supplierRes = await axios.get(
        endpoint.supplierActiveListUrl,
        endpoint.headers
      );
  ...
  const findSupplierDue = async (supplierId) => {
    try {
      setPreviousDue(0);
      setCurrentBalance(0);
      let data = new FormData();
      data.append("supplier_id", JSON.stringify(supplierId));
      const result = await axios.post(
        endpoint.supplierDueAPi,
        data,
        endpoint.headers
      );
